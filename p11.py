...
Find the biggest product of four consecutive numbers in a 20x20 grid.
...
#Note to self: python does not like to have zeroes in front of integers.
...
def horizontal_product(twenty):
	score = 0
	for z in range(20):
		for y in range(16):
			keeper = ((twenty[z][y])*(twenty[z][y+1])*(twenty[z][y+2])*(twenty[z][y+3]))
			if keeper > score:
			 	score = keeper
	return score
	
def vert_prod(twenty):
	score = 0
	for z in range(16):
		for y in range(20):
			keeper = ((twenty[z][y])*(twenty[z+1][y])*(twenty[z+2][y])*(twenty[z+3][y]))
			if keeper > score:
			 	score = keeper
	return score
	
def diagdown_score(twenty):
	score = 0
	for z in range(16):
		for y in range(16):
			keeper = ((twenty[z][y])*(twenty[z+1][y+1])*(twenty[z+2][y+2])*(twenty[z+3][y+3]))
			if keeper > score:
			 	score = keeper
	return score

def diagup_score(twenty):
	score = 0
	twenty.reverse()
	for z in range(16):
		for y in range(16):
...
print vert_prod(grid)
print diagdown_score(grid)
print diagup_score(grid)
